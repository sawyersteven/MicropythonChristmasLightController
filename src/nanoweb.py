# Slightly modified from https://github.com/hugokernel/micropython-nanoweb/blob/master/nanoweb.py
...
class Request:
    ...
    async def simple_response(self, code, reason):
        await self.write(f"HTTP/1.0 {code} {reason}\r\n\r\n{reason}")
            ...
            try:
                ...
                if request.url in self.routes:
                    ...
                    for route, handler in self.routes.items():
                        if route == request.url or (
                            route[-1] == "*" and request.url.startswith(route[:-1])
                        ):
                            request.route = route
